...
object App {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //   println(s"Cluster $i Centroid: ${centroids(i)}")
    // }
    val clusteredCount = clusteredData.count() 
    val clusterCounts = clusteredData.groupBy("prediction").count()
    val sortedClusterCounts = clusterCounts.sort("prediction")
    val result = sortedClusterCounts.collect()
    result.foreach(row => println(s"Cluster ${row.getAs[Int]("prediction")}: ${row.getAs[Long]("count")} data points"))
    println(s"total $clusteredCount data points")
