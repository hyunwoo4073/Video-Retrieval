...
//     dtwMatrix(n)(m)
//   }
//   def processRow(row: Row): ((Long, Long, Int), Vector) = {
...
//   def createPartitionedRDD(clusteredData: DataFrame, numClusters: Int): RDD[((Long, Long, Int), Vector)] = {
//     clusteredData
//       .rdd
//       .map(processRow) // 각 행을 처리하여 ((video_id, no_img, clusterId), orig_feature) 튜플로 변환
...
//     val partitionedRDD = createPartitionedRDD(clusteredData, numClusters)
