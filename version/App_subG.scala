...
object App {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val rawData: DataFrame = spark.read.parquet("/input/output3.parquet")
    rawData.printSchema()
    ...
    for (i <- 1 to iterations) {
      ...
      val results = queryFeatures.map { queryFeature =>
...
//     // set up a k-nn query
//     val queryFeatures = spark.sparkContext.broadcast(Array(
//       Vectors.dense(Array.fill(32)(math.random())),
//       Vectors.dense(Array.fill(32)(math.random())),
//       Vectors.dense(Array.fill(32)(math.random()))
//     ))
...
//     for (i <- 1 to iterations) {
//       val startTime = System.nanoTime()
//       val results = queryFeatures.value.map { queryFeature =>
//         // get 2 closest centroids to the query features
//         val closestClusterIds = broadcastCentroids.value.zipWithIndex.map { case (center, idx) =>
//           (idx, Vectors.sqdist(center, queryFeature))
//         }.sortBy(_._2).take(10).map(_._1)
