...
//     val queries = lines.flatMap(_.split(" "))
...
//         partitionOfRecords.foreach { query =>
//           val queryFeaturesArray = query.split(",").map(_.trim.toDouble)
...
//               val dtwDist =
//                 dtwDistance(queryFeatures, imageFeature.approxFeatures(0))
...
//               println(
//                 s"Video ID: ${imageFeature.video_Id}, DTW Distance: $dtwDist"
//               )
//             }
//           }
//         }
...
//         matchedGraphs.append(matchedClipGraph)
//         // 이벤트 정보 추출 (시작, 끝, 거리)
//         val eventStart = matchedClipGraph.head.no_img
//         val eventEnd = eventStart + windowSize - 1
...
object App {
  ...
  // bucketSize를 정해서 그 숫자만큼 근사화를 할 수 있게 변환하는 함수
  def ax_feature(features: Vector, numBuckets: Int): Vector = {
    val bucketSize = 1.0 / numBuckets
    val approxFeature = Vectors.dense(features.toArray.map { value =>
      val bucketIndex = math.min((value / bucketSize).toInt, numBuckets - 1)
  ...
  // 두 거리를 비교하는 것인데 시간을 고려하여 서로 얼마나 유사한지를 비교하는 함수
  // 가까우면 가까울수록 정확도가 높음
  def dtwDistance(x: Vector, y: Vector): Double = {
    val m = x.size
    val n = y.size
  ...
  def main(args: Array[String]): Unit = {
    val spark = SparkSession
      .builder()
      .master("yarn") // yarn으로 실행
      .appName("SimilaritySearch")
      .getOrCreate()

    val rawData: DataFrame =
      spark.read.parquet("/input/output2.parquet")
    ...
    //   }
    //   .toDF("video_Id", "similarity", "origFeatures", "no_img", "approxFeature")
    val approxFeaturesRDD = defaultImgFeatureRDD.map { imageFeature =>
      val videoId = imageFeature.video_Id
      val origFeatures = imageFeature.origFeatures
      val noImg = imageFeature.no_img
      val approxFeatures = imageFeature.approxFeatures.map(ax_feature(_, 5))
      (videoId, approxFeatures, origFeatures, noImg)
    }
    
    // var start = System.nanoTime() // 시작 시간 측정
    val similarities = approxFeaturesRDD.flatMap { case (videoId, approxFeatures, origFeatures, noImg) =>
      // val origFeatures = origFeatures // Assuming that origFeatures can be obtained similarly
      val similarities = approxFeatures.map(approxFeature =>
        compareFeatures(approxFeature, queryFeatures)
      )
      val rows = approxFeatures.zip(similarities).map {
        case (approxFeature, similarity) =>
          (
            ...
            similarity,
            origFeatures,
            noImg, // Assuming that no_img can be obtained similarly
            approxFeature
          )
      }
      rows
    ...
    // val (videoGraphs, event) =
    //   searchFV(clipsArray, queryFeatures, k, windowSize)
    // val top3Graphs = videoGraphs.take(10)
    // println("top10")
    // for (i <- 0 until top3Graphs.length) {
    //   val graph = top3Graphs(i)
    //   println(s"Graph $i:")
    //   for (imageFeature <- graph) {
    //     println(s"Image Feature: $imageFeature")
    //   }
    // }
    // top3Graphs.foreach { graph =>
    //   println(
...
//       val clipFeatures =
//         clipGraph.map(imgFeature => ax_feature(imgFeature.approxFeatures(0), 5))
//       val dist = clipFeatures.zipWithIndex.map { case (features, index) =>
//         dtwDistance(querySubgraph, features)
//       }.sum
...
//     val queryFeaturesArray = queryString.split(",").map(_.trim.toDouble)
//     Vectors.dense(queryFeaturesArray)
//   }
// }
