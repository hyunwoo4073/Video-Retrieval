...
// 결과 출력
  ...
  def processResults(results: Seq[(Long, Long, org.apache.spark.ml.linalg.Vector)]): Unit = {
    val groupedResults = results.grouped(5).toList
    groupedResults.foreach { group =>
      val featureList = group.map { case (videoId, noImg, features) =>
        // 각 feature에서 필요한 값을 추출하여 리스트에 추가
        // 예: features.toArray.toList
      }
      println(s"Grouped Features: $featureList")
    }
  }
}
